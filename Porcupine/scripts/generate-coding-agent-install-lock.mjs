...
const installPackageName = "@porcupineai/coding-agent-install";
...
// entry of their own) are expected to be absent from the lock.
...
function resolveExternalDependency(lockPackages, packageName, fromLockPath) {
	...
	// Self-contained npm link entries (e.g. moonshine-js's bundled VAD) resolve
	// inside the dependency's own tree; accept them so the walk does not fail.
	for (const directory of candidateDirs) {
		const candidate = directory ? `${directory}/node_modules/${packageName}` : `node_modules/${packageName}`;
		const entry = lockPackages[candidate];
		if (entry?.link) {
