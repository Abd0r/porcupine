...
const internalPackagePrefix = "@porcupineai/";
// npm link dependencies that ship inside their parent package (no registry
// entry of their own) are expected to be absent from the shrinkwrap.
const allowedLinkDeps = new Set(["@ricky0123/vad-web"]);
...
function resolveExternalDependency(lockPackages, packageName, fromLockPath) {
	...
	// npm "link" entries resolve inside the dependency's own tree (e.g.
	// @moonshine-ai/moonshine-js's bundled VAD). Accept them as a fallback;
	// addExternalPackage skips the copy so they never land in the shrinkwrap.
	for (const directory of candidateDirs) {
		const candidate = directory ? `${directory}/node_modules/${packageName}` : `node_modules/${packageName}`;
		const entry = lockPackages[candidate];
		if (entry?.link) {
...
function addExternalPackage(lockPackages, shrinkwrapPackages, addedPaths, queue, name, from) {
	...
	// Self-contained link entries (bundled inside their parent package) have no
	// registry presence; they are already covered by the parent's own tarball.
	if (entry.link) {
...
function validateShrinkwrap(shrinkwrap, internalNames) {
	...
	for (const [lockPath, entry] of Object.entries(shrinkwrap.packages)) {
		...
		// npm link entries (e.g. a dependency that symlinks into its own package
		// tree, like @moonshine-ai/moonshine-js's bundled VAD) are self-contained
		// and safe for publishing — the target ships inside the dependency.
		if (entry.link) {
			continue;
		}
	...
	for (const [lockPath, entry] of Object.entries(shrinkwrap.packages)) {
		for (const dependencyName of Object.keys(packageDependencies(entry))) {
			...
			if (allowedLinkDeps.has(dependencyName)) {
				continue;
			}
