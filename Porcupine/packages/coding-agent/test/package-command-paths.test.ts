...
describe("package commands", () => {
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".porcupine"), { recursive: true });
		writeFileSync(
			join(projectDir, ".porcupine", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".porcupine"), { recursive: true });
		writeFileSync(join(projectDir, ".porcupine", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".porcupine", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".porcupine", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("porcupine install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain(
				'Use "porcupine --help" or "porcupine install <source> [-l] [--approve|--no-approve]".',
			);
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: porcupine install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		...
		const previousLatestUrl = process.env.PORCUPINE_LATEST_VERSION_URL;
		process.env.PORCUPINE_LATEST_VERSION_URL = "https://porcupine.invalid/version";
		...
		try {
			...
			expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
				`porcupine is already up to date (v${VERSION})`,
			);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
			...
			if (previousLatestUrl === undefined) {
				delete process.env.PORCUPINE_LATEST_VERSION_URL;
			} else {
				process.env.PORCUPINE_LATEST_VERSION_URL = previousLatestUrl;
			}
			process.env.PI_SKIP_VERSION_CHECK = previousSkipVersionCheck;
		}
	});
	...
	it("uses the update check version for forced self updates even when current", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@porcupine", "coding-agent");
...
`,
		...
		try {
			...
			expect(stdout).toContain(`Updated porcupine from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated porcupine from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		const selfPackageDir = join(globalRoot, "node_modules", "@porcupine", "coding-agent");
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated porcupine");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `porcupine update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
