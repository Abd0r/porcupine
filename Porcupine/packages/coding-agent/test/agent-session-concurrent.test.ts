...
describe("AgentSession concurrent prompt guard", () => {
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const agent = new Agent({
			...
			streamFn: (_model, context, options) => {
				...
				queueMicrotask(() => {
					...
					if (userTexts.some((t) => t.includes("Steer from extension"))) {
		...
		// sendUserMessage via extension API is fire-and-forget (doesn't return a Promise);
		// poll until the message is queued.
		for (let i = 0; i < 10 && session.pendingMessageCount === 0; i++) {
			await new Promise((resolve) => setTimeout(resolve, 10));
		}

		expect(session.pendingMessageCount).toBe(1);
		const msgs = session.getSteeringMessages();
		expect(msgs.some((m) => m.includes("Steer from extension"))).toBe(true);
		expect(lastInputSource).toBe("extension");
		expect(queueEvents.some((event) => event.steering.some((m) => m.includes("Steer from extension")))).toBe(true);
