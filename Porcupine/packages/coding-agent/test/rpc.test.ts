...
import type { AgentEvent } from "@porcupineai/agent-core";
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	...
	test("should get available thinking levels", async () => {
		...
		// The current fixed level must be in the available list (adaptive is extra)
		const state = await client.getState();
		const fixedLevels = levels.filter((l) => l !== "adaptive");
		expect(fixedLevels).toContain(state.thinkingLevel);
		expect(levels).toContain("adaptive");

		// cycle_thinking_level stays on the full ring (fixed + adaptive)
