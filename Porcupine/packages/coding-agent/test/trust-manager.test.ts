...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".porcupine", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".porcupine", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".porcupine", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".porcupine"), { recursive: true });
			writeFileSync(join(cwd, ".porcupine", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".porcupine"), { recursive: true, force: true });
