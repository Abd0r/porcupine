...
import type { AssistantMessage, Usage } from "@porcupineai/ai/compat";
...
describe("shouldCompact", () => {
	...
	it("defaults to 80/20 of the context window when reserveTokens is unset", () => {
		// 20% of 200k = 40k reserve → threshold at 160k
		const settings: CompactionSettings = {
			enabled: true,
			keepRecentTokens: 20000,
		};

		expect(shouldCompact(160_001, 200_000, settings)).toBe(true);
		expect(shouldCompact(160_000, 200_000, settings)).toBe(false);
		// Fixed 16k reserve would not have triggered here — this is why auto felt broken.
		expect(shouldCompact(170_000, 200_000, { ...settings, reserveTokens: 16384 })).toBe(false);
	});

	it("DEFAULT_COMPACTION_SETTINGS uses the 80/20 rule", () => {
		expect(DEFAULT_COMPACTION_SETTINGS.reserveTokens).toBeUndefined();
		expect(shouldCompact(81_000, 100_000, DEFAULT_COMPACTION_SETTINGS)).toBe(true);
		expect(shouldCompact(80_000, 100_000, DEFAULT_COMPACTION_SETTINGS)).toBe(false);
	});
});
...
describe("Large session fixture", () => {
	...
	it("should find cut point in large session", () => {
		const entries = loadLargeSessionEntries();
		const result = findCutPoint(entries, 0, entries.length, DEFAULT_COMPACTION_KEEP_MIN);
