...
describe("experimental CLI command composition", () => {
	test("composes porcupine command options with the existing parser", () => {
	...
	test.each(["--help", "--version"] as const)("keeps Porcupine %s handling in existing CLI options", (option) => {
		expect(experimentalCli.parse([option])).toMatchObject({
			ok: true,
			command: { command: "porcupine", options: { [option === "--help" ? "help" : "version"]: true } },
		});
	});
	...
	test.each(["porcupine", "server", "client"] as const)("executes the parsed %s command", async (name) => {
		const context = {
			runPorcupine: vi.fn(() => undefined),
			runServer: vi.fn(() => undefined),
			runClient: vi.fn(() => undefined),
		};
		const result = await experimentalCli.execute(name === "porcupine" ? [] : [name], context);

		expect(result).toMatchObject({ ok: true, command: { command: name } });
		expect(context.runPorcupine).toHaveBeenCalledTimes(name === "porcupine" ? 1 : 0);
