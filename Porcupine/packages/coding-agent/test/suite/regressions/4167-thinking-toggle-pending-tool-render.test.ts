...
import type { AssistantMessage, ToolResultMessage, Usage } from "@porcupineai/ai";
import { Container, Text, type TUI } from "@porcupineai/tui";
...
type RenderSessionContextThis = {
	...
	/** Extension tool components hidden while the thinking toggle is off. */
	extensionToolComponentsHidden: boolean;
	learningToolEvidence: Array<{ name: string; isError: boolean }>;
	orchestrator: {
		markToolFinished(): void;
		markStepForTool?(toolName: string): void;
		getTaskGraph(): { nodes: unknown[]; edges: unknown[] };
	};
...
function createFakeInteractiveModeThis(): RenderSessionContextThis {
	...
	return {
		...
		isInitialized: true,
		extensionToolComponentsHidden: false,
		learningToolEvidence: [] as Array<{ name: string; isError: boolean }>,
		orchestrator: { markToolFinished: vi.fn(), getTaskGraph: () => ({ nodes: [], edges: [] }) },
