import { fauxAssistantMessage } from "@porcupineai/ai";
import { Container, Text } from "@porcupineai/tui";
...
describe("regression #5943: session_start transient UI", () => {
	...
	it("renders replacement session state before session_start handlers can notify", async () => {
		...
		try {
			...
			expect(events).toEqual(["apply", "confirm", "render", "subscribe", "bind", "notify:Hello Error"]);
		} finally {
			harness.cleanup();
		}
	});
	...
	it("subscribes before replacement session_start handlers send user messages", async () => {
		...
		try {
			...
			expect(events.slice(0, 4)).toEqual(["confirm", "render", "subscribe", "bind"]);
