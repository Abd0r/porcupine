...
import { fauxAssistantMessage, fauxToolCall, registerFauxProvider } from "@porcupineai/ai/compat";
...
describe("AgentSessionRuntime characterization", () => {
	...
	it("refresh force-flushes, rebuilds runtime, and resumes the same session", async () => {
		...
		await runtime.session.bindExtensions({});
		// User-only turn: file path exists but is not on disk until force-flush.
		...
		expect(runtime.session.sessionFile).toBe(originalFile);
		expect(existsSync(originalFile!)).toBe(true);
		expect(runtime.session.messages.some((m) => m.role === "user")).toBe(true);
		expect(events.map((e) => e.type)).toEqual(["session_before_switch", "session_shutdown", "session_start"]);
		expect(events[0]).toMatchObject({ type: "session_before_switch", reason: "resume" });
		expect(events[2]).toMatchObject({ type: "session_start", reason: "resume" });
	});
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		expect(successResult.selectedText).toContain("hello");
		await runtime.session.bindExtensions({});
		expect(events).toEqual([
