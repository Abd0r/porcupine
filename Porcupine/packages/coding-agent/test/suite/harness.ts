...
import type { AgentMessage, AgentTool } from "@porcupineai/agent-core";
import { Agent } from "@porcupineai/agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@porcupineai/ai/compat";
import { registerFauxProvider, streamSimple } from "@porcupineai/ai/compat";
...
/** Legacy stripper: date/time used to be prepended to every user prompt; now only system/compaction. */
const DATE_TIME_PREFIX_RE = /^\[Current date\/time:[^\n]*\n/;
...
export function getMessageText(message: unknown): string {
	...
	if (typeof content === "string") {
		return content.replace(DATE_TIME_PREFIX_RE, "");
	}
	return content
		.filter((part): part is MessageTextPart => part.type === "text")
		.map((part) => part.text.replace(DATE_TIME_PREFIX_RE, ""))
		.join("\n");
}
