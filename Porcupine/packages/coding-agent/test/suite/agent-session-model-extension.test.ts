import type { AgentTool, ThinkingLevel } from "@porcupineai/agent-core";
import { fauxAssistantMessage, fauxToolCall, type Model, type Usage } from "@porcupineai/ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("cycles xhigh before max, then adaptive after max (Shift+Tab ring)", async () => {
		...
		expect(harness.session.getAvailableReasoningModes()).toEqual([
			"off",
			"minimal",
			"low",
			"medium",
		...
		expect(harness.session.cycleThinkingLevel()).toBe("adaptive");
		expect(harness.session.isAdaptiveReasoningEnabled).toBe(true);
		expect(harness.session.cycleThinkingLevel()).toBe("off");
		expect(harness.session.isAdaptiveReasoningEnabled).toBe(false);
	});
	...
	it("allows extension context handlers to modify messages before the LLM call", async () => {
		...
		if (storedUserMessage?.role === "user") {
			expect(storedUserMessage.content).toEqual([{ type: "text", text: expect.stringContaining("original") }]);
		}
	});
	...
	it("allows extension input handlers to transform or handle input", async () => {
		...
		expect(providerUserText).toContain("transformed:hello");
