...
import { Text, type TUI } from "@porcupineai/tui";
	...
	for (const scenario of [
		{
			...
			compact: "[skill] attio",
			// By design the collapsed SKILL.md read shows a compact body preview
			// (skills are never a blank tool call); the instructions field is gone.
			hidden: undefined,
			previewVisible: "# Hidden skill instructions",
			absent: "read skill attio",
		},
		{
			title: "AGENTS.md",
			path: join(process.cwd(), ".porcupine", "AGENTS.md"),
			content: "Hidden resource instructions",
			compact: "read resource .porcupine/AGENTS.md",
		...
		test(`renders ${scenario.title} read results compactly until expanded`, () => {
			...
			if (scenario.previewVisible) {
				expect(collapsed).toContain(scenario.previewVisible);
			...
			component.setExpanded(true);
			const expanded = stripAnsi(component.render(120).join("\n"));
			expect(expanded).toContain(scenario.hidden ?? scenario.previewVisible);
		});
	}
