...
Content`,
		...
		it("should auto-discover root markdown skills from .porcupine skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .porcupine", async () => {
			const extDir = join(tempDir, ".porcupine", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".porcupine"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".porcupine", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".porcupine", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".porcupine", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".porcupine", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".porcupine", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		...
		it("should use the project .porcupine dir as baseDir for project .porcupine skills", async () => {
			const projectBaseDir = join(tempDir, ".porcupine");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".porcupine", "agent");
		...
		it("should dedupe user skill entries when ~/.porcupine/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .porcupine auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".pi\n");

			const skillDir = join(tempDir, ".porcupine", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".porcupine", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .porcupine settings base", () => {
			...
			const rel = relative(join(tempDir, ".porcupine"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{
					source: relative(join(tempDir, ".porcupine"), pkgDir),
					autoload: false,
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".porcupine", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".porcupine", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".porcupine", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".porcupine", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".porcupine", "npm", "node_modules", "example");
