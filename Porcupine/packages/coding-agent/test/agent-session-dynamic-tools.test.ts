...
describe("AgentSession dynamic tool registration", () => {
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(sessionEnv).toMatchObject({
			PORCUPINE_SESSION_ID: session.sessionId,
			PORCUPINE_SESSION_FILE: session.sessionFile,
			PORCUPINE_PROVIDER: model.provider,
			PORCUPINE_MODEL: model.id,
			PORCUPINE_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("PORCUPINE_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("PORCUPINE_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("PORCUPINE_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("PORCUPINE_MODEL");
		expect(optedOutEnv).not.toHaveProperty("PORCUPINE_REASONING_LEVEL");
