...
Skill content here.`,
		...
		it("should discover bundled Porcupine skills by default", async () => {
			const loader = new DefaultResourceLoader({ cwd, agentDir });
			await loader.reload();

			const { skills } = loader.getSkills();
			expect(skills.some((skill) => skill.name === "source-driven-development")).toBe(true);
		});

		it("should exclude bundled skills when skill discovery is disabled", async () => {
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".porcupine", "prompts");
			...
			const projectSkillDir = join(cwd, ".porcupine", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".porcupine", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".porcupine", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".porcupine"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".porcupine", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".porcupine", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".porcupine", "extensions");
...
export default function(pi) {
	...
	pi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".porcupine", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.pi", async () => {
			const piDir = join(cwd, ".porcupine");
