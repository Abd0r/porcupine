...
import { type AssistantMessage, createAssistantMessageEventStream, fauxAssistantMessage } from "@porcupineai/ai";
import { getModel, streamSimple } from "@porcupineai/ai/compat";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { AgentSession } from "../src/core/agent-session.ts";
import { AuthStorage } from "../src/core/auth-storage.ts";
import { DEFAULT_COMPACTION_KEEP_MIN, resolveCompactionKeepRecentTokens } from "../src/core/compaction/index.ts";
...
describe("AgentSession auto-compaction queue resume", () => {
	...
	it("resolves keepRecentTokens against the model context window when unset (not the 8k floor)", () => {
		// Default compaction settings leave keepRecentTokens unset → the 80/20 rule
		// must scale it to the active model's window (clamped 8k–80k). Regression
		// guard: prepareCompaction receives keepRecentTokens=0 when the session
		// forgot to pre-resolve, collapsing every model to the 8k floor.
		const resolved = (
			session as unknown as {
				_resolvedCompactionSettings(): { enabled: boolean; reserveTokens?: number; keepRecentTokens: number };
			}
		)._resolvedCompactionSettings();

		const expected = resolveCompactionKeepRecentTokens(session.model!.contextWindow ?? 0, undefined);
		expect(resolved.keepRecentTokens).toBe(expected);
		expect(resolved.keepRecentTokens).toBeGreaterThan(DEFAULT_COMPACTION_KEEP_MIN);
	});
	...
	it("should trigger threshold compaction for error messages using last successful usage", async () => {
		...
		// Default reserve is 20% of the model context window (80/20 rule).
		const contextWindow = model.contextWindow ?? 200_000;
		const reserveTokens = Math.round(contextWindow * 0.2);
		const thresholdTokens = contextWindow - reserveTokens + 1;
