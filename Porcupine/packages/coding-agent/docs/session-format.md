...
```
~/.porcupine/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.porcupine/agent/sessions/`.

Porcupine also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, porcupine uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([porcupine](https://github.com/Abd0r/porcupine)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/@porcupineai/coding-agent/dist/` and `node_modules/@porcupineai/ai/dist/`.
...
### Base Message Types (from @porcupineai/ai)
...
The exported @porcupineai/ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before porcupine persists the assistant message, so `"pending"` should never appear in session JSONL.

### Extended Message Types (from @porcupineai/coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if porcupine-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `api.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `porcupine.setSessionName()` in extensions.
