...
Use `/logout` to clear credentials. Tokens are stored in `~/.porcupine/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.porcupine/agent/auth.json`:
...
Use this when porcupine should use different provider settings than the project shell environment.
...
```bash
porcupine --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
porcupine --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
porcupine --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal porcupine usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
porcupine --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Porcupine automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
...
Porcupine supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage loaded models with `/llama`, and select a loaded model with `/model`.
