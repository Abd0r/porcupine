...
Porcupine saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.porcupine/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
porcupine -c                  # Continue most recent session
porcupine -r                  # Browse and select from past sessions
porcupine --no-session        # Ephemeral mode; do not save
porcupine --name "my task"    # Set session display name at startup
porcupine --session <path|id> # Use a specific session file or partial session ID
porcupine --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `porcupine -r` opens the same picker at startup.
...
When available, porcupine uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
porcupine --name "Refactor auth module"
porcupine --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `porcupine -r`.
...
When `/tree` switches away from one branch to another, porcupine can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
