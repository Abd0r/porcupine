...
Extensions can register either a complete @porcupineai/ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Porcupine composes `models.json` overrides above registered native providers.
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. porcupine waits for the factory before startup continues, so the provider is available during interactive startup and to `porcupine --list-models`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map porcupine levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@porcupineai/ai";
...
Credentials are persisted in `~/.porcupine/agent/auth.json`:
...
- [anthropic.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, porcupine can recover automatically by compacting the conversation and retrying. This recovery only kicks in if porcupine recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of porcupine's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message porcupine does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase porcupine recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before porcupine tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what porcupine checks. With this in place, porcupine will:
...
- Match a provider-specific pattern, not porcupine's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of porcupine's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/Abd0r/porcupine/tree/main/Porcupine/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps porcupine thinking levels to provider/model-specific values; null marks a level unsupported. */
