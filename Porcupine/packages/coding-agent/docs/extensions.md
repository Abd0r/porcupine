> porcupine can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend porcupine's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.
...
Create `~/.porcupine/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.porcupine/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.porcupine/agent/extensions/*.ts` | Global (all projects) |
| `~/.porcupine/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.porcupine/extensions/*.ts` | Project-local |
| `.porcupine/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as porcupine packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@porcupineai/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@porcupineai/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@porcupineai/tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed porcupine packages installed with `porcupine install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
This pattern makes the fetched models available during normal startup and to `porcupine --list-models`.
...
/name or porcupine.setSessionName()
...
thinking level changes (settings, keybinding, porcupine.setThinkingLevel())
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether porcupine asks, trusts, or declines by default.
...
After a successful switch or new-session action, porcupine emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork or clone, porcupine emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
pi.on("before_agent_start", async (event, ctx) => {
  ...
  return {
    ...
    // Replace the user prompt text for this turn (e.g. OCR-injected image descriptions)
    // When returned, the main model receives only this text — attached images are stripped.
    prompt: event.prompt + "\n\n[image-extracted context here]",
  };
});
```

The `systemPromptOptions` field gives extensions access to the same structured data Porcupine uses to build the system prompt. This lets you inspect what Porcupine has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
pi.on("before_provider_headers", (event, ctx) => {
  ...
  // Drop a tracking header porcupine adds for this call
...
Use this to update extension UI when `porcupine.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, porcupine waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
pi.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap porcupine's built-in local bash backend
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective @porcupineai/ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
Control flow helpers. `ctx.isIdle()` is false while Porcupine is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of porcupine.
...
Returns Porcupine's current system prompt string.
...
Returns the base inputs Porcupine currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `porcupine` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
- Captured old `porcupine` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
  ...
  handler: async (_args, ctx) => {
    ...
    await ctx.newSession({
      withSession: async (_ctx) => {
        ...
        porcupine.sendUserMessage("wrong");
      },
    });
  },
});
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      porcupine.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`api.appendEntry()`](#piappendentrycustomtype-data) with [`api.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### porcupine.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
porcupine.sendUserMessage("What is 2+2?");

// With content array (text + images)
porcupine.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
porcupine.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
porcupine.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### porcupine.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `api.registerEntryRenderer()`.

```typescript
porcupine.appendEntry("my-state", { count: 42 });
...
### porcupine.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
porcupine.setSessionName("Refactor auth module");
```

### porcupine.getSessionName()

Get the current session name, if set.

```typescript
const name = porcupine.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### porcupine.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
porcupine.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
porcupine.setLabel(entryId, undefined);
...
### porcupine.getCommands()

Get the slash commands available for invocation via `prompt` in the current session. Includes extension commands, prompt templates, and skill commands.
The list matches the RPC `get_commands` ordering: extensions first, then templates, then skills.

```typescript
const commands = porcupine.getCommands();
...
Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `api.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### api.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Porcupine renders the transformed content with its built-in renderer.
...
```typescript
api.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, Porcupine keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### api.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `porcupine.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@porcupineai/tui";

api.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
// Check value
if (porcupine.getFlag("plan")) {
  // Plan mode enabled
}
```

### porcupine.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await porcupine.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### porcupine.getActiveTools() / porcupine.getAllTools() / porcupine.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `porcupine.getActiveTools()` returns the active tool names as `string[]`; `porcupine.getAllTools()` returns metadata for all configured tools.

```typescript
const active = porcupine.getActiveTools(); // ["read", "bash", ...]
const all = porcupine.getAllTools();
// all = [{
//   name: "read",
...
porcupine.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
porcupine.setActiveTools(["read", "bash"]); // Switch to read-only
```

`porcupine.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await porcupine.setModel(model);
  if (!success) {
...
### porcupine.getThinkingLevel() / porcupine.setThinkingLevel(level)
...
```typescript
const current = porcupine.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
porcupine.setThinkingLevel("high");
```
...
```typescript
porcupine.events.on("my:event", (data) => { ... });
...
Dynamic providers can implement `refreshModels`. Porcupine calls it during model refresh, publishes the returned list synchronously through the provider, and passes the canonical credential/store/network/signal context. The extension decides whether to persist the catalog through `context.store`; live servers such as llama.cpp can ignore it.

Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@porcupineai/ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@porcupineai/ai";
...
The object form accepts a complete @porcupineai/ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `porcupine.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@porcupineai/coding-agent";
...
pi.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via porcupine.exec (captured from extension closure)
    const result = await porcupine.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Porcupine persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@porcupineai/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when porcupine resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
porcupine -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
porcupine --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@porcupineai/coding-agent";
...
For `user_bash`, extensions can reuse porcupine's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The bash tool also supports a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "@porcupineai/coding-agent";
...
`createBashTool()` exposes the current session through `PORCUPINE_SESSION_*` / `PORCUPINE_PROVIDER` / `PORCUPINE_MODEL` / `PORCUPINE_REASONING_LEVEL` (also mirrored as `PI_*`). Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/Abd0r/porcupine/blob/main/Porcupine/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@porcupineai/coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `porcupine.setActiveTools()` during execution. Porcupine detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
5. Before the next model response, Porcupine exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `porcupine.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Porcupine adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Porcupine sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Porcupine also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = porcupine.getAllTools()
      ...
      const active = porcupine.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      porcupine.setActiveTools([...new Set([...active, ...added])]);
  ...
  pi.on("session_start", () => {
    ...
    porcupine.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("porcupine - my-project");
...
```typescript
import { Text, Component } from "@porcupineai/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@porcupineai/coding-agent";
import { matchesKey } from "@porcupineai/tui";
...
api.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `api.sendMessage()`:
...
```typescript
api.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

porcupine.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@porcupineai/coding-agent";
...
| `event-bus.ts` | Inter-extension events | `porcupine.events` |
| **Session Metadata** |||
