> porcupine can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to porcupine's agent capabilities. Use it to embed porcupine in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession, ModelRuntime, SessionManager } from "@porcupineai/coding-agent";
...
```bash
npm install @porcupineai/coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@porcupineai/coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `api.sendMessage()`.
...
The `Agent` class (from `@porcupineai/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.porcupine/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.porcupine/extensions/`)
- Project skills:
  - `.porcupine/skills/`
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.porcupine/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@porcupineai/ai";
import { ModelRuntime } from "@porcupineai/coding-agent";
...
```typescript
import { InMemoryCredentialStore } from "@porcupineai/ai";
import { createAgentSession, ModelRuntime } from "@porcupineai/coding-agent";

// Default: uses ~/.porcupine/agent/auth.json and ~/.porcupine/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any @porcupineai/ai CredentialStore
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@porcupineai/coding-agent";
...
- Built-in tool names: `ask_question`, `read`, `bash`, `edit`, `write`, `grep`, `find`, `ls`, `web_search`, `web_extract`, `computer_use`, `capability_search`, `memory`, `session_search`
- Default built-ins: `read`, `bash`, `edit`, `write` (plus Porcupine interaction and research tools when using the standard runtime)
- `noTools: "all"` disables all tools
- `noTools: "builtin"` disables default built-ins while keeping extension and custom tools enabled
- `excludeTools` disables specific built-in, extension, or custom tool names after any `tools` allowlist is applied

The `edit` tool returns `details.diff` for Porcupine's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@porcupineai/coding-agent";
...
import { createAgentSession, defineTool } from "@porcupineai/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.porcupine/agent/extensions/`, `.porcupine/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@porcupineai/coding-agent";
...
**Event Bus:** Extensions can communicate via `porcupine.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@porcupineai/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@porcupineai/coding-agent";
...
1. Global: `~/.porcupine/agent/settings.json`
2. Project: `<cwd>/.porcupine/settings.json`
...
// Auth and Models
ModelRuntime // implements @porcupineai/ai Models and owns credential storage
