...
The `apiKey` value is a placeholder because Ollama ignores it. porcupine still treats models as requiring auth before they appear in `/model`, so keyless local servers should keep a dummy value, save a key for that provider with `/login`, or pass `--api-key` when selecting the model.

Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so porcupine sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
Models whose catalog entry declares `audio` input receive recorded voice
directly (no STT round-trip) when Voice Mode is on. This includes **Gemini 2.5+/3.x**,
and **Inkling** (Thinking Machines, via Together AI — `ThinkingMachines/Inkling`),
an open-weights 975B/41B MoE that natively reasons over text, image, and audio.
Text-only models fall back to Moonshine STT + Kokoro TTS.

## Recommended sub-agent model

Sub-agents (the `subagent` tool) default to the parent model. For cheap, fast delegation, set:

```json
{ "subagent": { "model": "opencode-go/deepseek-v4-flash" } }
```

DeepSeek V4 Flash through the OpenCode Go provider offers a 1M context window at a small fraction of flagship pricing — see [settings.md](settings.md) for all `subagent.*` options.
...
For `models.json`, shell commands are resolved at request time. porcupine intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and porcupine cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps porcupine thinking levels to provider values and marks unsupported levels (see below) |
...
Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are porcupine thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Maps may contain holes; for example, a model can expose `high` and `max` without exposing `xhigh`.
...
By default porcupine sends per-tool `eager_input_streaming: true`. If a proxy or Anthropic-compatible backend rejects that field, set `supportsEagerToolInputStreaming` to `false`. Porcupine will omit `tools[].eager_input_streaming` and send the legacy `fine-grained-tool-streaming-2025-05-14` beta header for tool-enabled requests instead.
...
| `supportsFinishReason` | Whether streamed responses include `finish_reason`. When `false`, porcupine infers `stop` or `toolUse` when the stream ends. Default: `true`. |
