> porcupine can create themes. Ask it to build one for your setup.
...
On first run, porcupine detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.porcupine/agent/themes
vim ~/.porcupine/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Abd0r/porcupine/main/Porcupine/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, porcupine reloads it automatically for immediate visual feedback.
...
Porcupine uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, porcupine falls back to the nearest approximation.
