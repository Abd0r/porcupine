> porcupine can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@porcupineai/tui`](https://github.com/Abd0r/porcupine/tree/main/Porcupine/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@porcupineai/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `PORCUPINE_HARDWARE_CURSOR=1` / `PI_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@porcupineai/tui";
...
Import from `@porcupineai/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@porcupineai/tui";
```
...
```typescript
import { matchesKey, Key } from "@porcupineai/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@porcupineai/tui";
...
```typescript
import { getMarkdownTheme } from "@porcupineai/coding-agent";
import { Markdown } from "@porcupineai/tui";
...
Set `PORCUPINE_TUI_WRITE_LOG` (or legacy `PI_TUI_WRITE_LOG`) to capture the raw ANSI stream written to stdout.
...
For letting users pick from a list of options. Use `SelectList` from `@porcupineai/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@porcupineai/coding-agent";
import { DynamicBorder } from "@porcupineai/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@porcupineai/tui";
...
```typescript
import { BorderedLoader } from "@porcupineai/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@porcupineai/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@porcupineai/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@porcupineai/tui";
...
Customize the inline working indicator shown while porcupine is streaming a response.
...
// Restore porcupine's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@porcupineai/tui";
