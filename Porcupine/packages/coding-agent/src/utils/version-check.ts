...
export async function getLatestPiRelease(
	...
	// Porcupine does not inherit the upstream version endpoint. Only check when
	// an explicit product URL is configured.
	const latestVersionUrl = getProductEnvironment("LATEST_VERSION_URL");
	if (!latestVersionUrl) return undefined;

	const response = await fetch(latestVersionUrl, {
		headers: {
			"User-Agent": getPorcupineUserAgent(currentVersion),
