...
import { Markdown, type MarkdownTheme } from "@porcupineai/tui";
...
Update Porcupine, installed packages, or model catalogs.

Options:
  --self                  Update Porcupine only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update Porcupine and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall Porcupine even if the current version is latest

Short forms:
  ${APP_NAME} update                Update Porcupine only
  ${APP_NAME} update --all          Update Porcupine and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update ${APP_NAME}    Update Porcupine only (self works as alias)
`);
			return;
