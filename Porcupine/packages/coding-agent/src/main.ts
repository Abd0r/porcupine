...
import { type ImageContent, modelsAreEqual } from "@porcupineai/ai";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(getProductEnvironment("OFFLINE"));
	if (offlineMode) {
		setProductEnvironment("OFFLINE", "1");
		setProductEnvironment("SKIP_VERSION_CHECK", "1");
	}
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `porcupine update` drain naturally instead.
	...
	const startupBenchmark = isTruthyEnvFlag(getProductEnvironment("STARTUP_BENCHMARK"));
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: PORCUPINE_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
