...
export function getPackageDir(): string {
	...
	const envDir = getProductEnvironment("PACKAGE_DIR");
	if (envDir) {
	...
	// Node.js: walk up to the package root. Standalone-binary packaging copies a
	// package.json into dist/, which is an asset directory rather than a Node
	// package root. Require a source or distribution directory to avoid resolving
	// themes as dist/dist/modes/....
	let dir = __dirname;
	while (dir !== dirname(dir)) {
		if (existsSync(join(dir, "package.json")) && (existsSync(join(dir, "src")) || existsSync(join(dir, "dist")))) {
			return dir;
		}
		dir = dirname(dir);
	}
...
/**
 * Get the product skills shipped with Porcupine.
 *
 * Node installs keep these at the package root. The standalone Bun binary
 * copies the same directory beside its executable into dist/.
 */
...
export const PACKAGE_NAME: string = pkg.name || "@porcupineai/coding-agent";
export const APP_NAME: string = piConfigName || "porcupine";
export const APP_TITLE: string = pkg.piConfig?.title || (piConfigName ? APP_NAME : "Porcupine");
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".porcupine";
export const VERSION: string = pkg.version || "0.0.0";

// e.g., PORCUPINE_CODING_AGENT_DIR or TAU_CODING_AGENT_DIR
...
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = getProductEnvironment("SHARE_VIEWER_URL");
	if (!baseUrl) {
		throw new Error("Sharing requires PORCUPINE_SHARE_VIEWER_URL to be configured.");
	}
	return `${baseUrl}#${gistId}`;
}

// =============================================================================
// User Config Paths (~/.porcupine/agent/*)
// =============================================================================

/** Get the agent config directory (e.g., ~/.porcupine/agent/) */
