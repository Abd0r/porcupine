...
import type { AgentMessage, ThinkingLevel } from "@porcupineai/agent-core";
...
export class RpcClient {
	...
	/**
	 * Cycle thinking / reasoning mode (fixed levels then adaptive).
	 */
	async cycleThinkingLevel(): Promise<{ level: ThinkingLevel | "adaptive" } | null> {
		const response = await this.send({ type: "cycle_thinking_level" });
		return this.getData(response);
	}

	/**
	 * Full Shift+Tab ring for the current model (fixed levels + adaptive).
	 */
	async getAvailableThinkingLevels(): Promise<Array<ThinkingLevel | "adaptive">> {
		const response = await this.send({ type: "get_available_thinking_levels" });
		return this.getData<{ levels: Array<ThinkingLevel | "adaptive"> }>(response).levels;
	}
