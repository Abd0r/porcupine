...
export interface SettingsConfig {
	...
	/** When true, footer/settings show Adaptive as the active reasoning mode. */
...
export interface SettingsCallbacks {
	...
	onThinkingLevelChange: (level: ThinkingLevel | "adaptive") => void;
	...
	constructor(config: SettingsConfig, callbacks: SettingsCallbacks) {
		...
		const items: SettingItem[] = [
			...
			},
			{
				...
				description: "Reasoning depth for thinking-capable models (/reasoning)",
				currentValue: config.adaptiveReasoning ? "adaptive" : config.thinkingLevel,
				submenu: (currentValue, done) =>
					new SelectSubmenu(
						"Reasoning Mode",
						"Select fixed depth, or adaptive (per-turn). Also: /reasoning",
						[
							...
							})),
							{
								value: "adaptive",
								label: "adaptive",
								description: "Per-turn depth (model-picked: minimal…max)",
							},
						],
						currentValue,
						(value) => {
							callbacks.onThinkingLevelChange(value === "adaptive" ? "adaptive" : (value as ThinkingLevel));
							done(value);
						},
						() => done(),
					),
