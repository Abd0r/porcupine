...
import type { Component, TUI } from "@porcupineai/tui";
...
export class ArminComponent implements Component {
	...
	constructor(ui: TUI, options?: { effect?: Effect }) {
		this.ui = ui;
		// An explicit effect can be injected for deterministic testing; otherwise a random one is picked.
		this.effect = options?.effect ?? EFFECTS[Math.floor(Math.random() * EFFECTS.length)];
	...
	private tickRain(): boolean {
		...
		for (let x = 0; x < WIDTH; x++) {
			...
			// Find the target row for this column (lowest non-space pixel above the settled region)
			let targetRow = -1;
			...
			// If no non-space pixel remains above the settled region, the column is complete:
			// the settled-pixel loop above already drew every non-space pixel for this column.
			// Without this, columns whose topmost pixel sits above row 0 never reach
			// `settled >= DISPLAY_HEIGHT` and the drop falls forever through empty space.
			if (targetRow < 0) {
