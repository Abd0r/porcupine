...
const ThemeJsonSchema = Type.Object({
	...
	colors: Type.Object({
		...
		// Thinking Level Borders (6 fixed levels plus optional adaptive)
		...
		thinkingAdaptive: Type.Optional(ColorValueSchema),
		// Bash Mode (1 color)
...
function withThemeColorFallbacks(colors: ThemeJson["colors"]): ThemeJson["colors"] & {
	...
	return {
		...
		thinkingAdaptive: colors.thinkingAdaptive ?? colors.thinkingHigh,
...
export class Theme {
	...
	constructor(
		...
		const colors = {
			...
			thinkingAdaptive: fgColors.thinkingAdaptive ?? fgColors.thinkingHigh,
		};
	...
	getAdaptiveThinkingBorderColor(): (str: string) => string {
		return (str: string) => this.fg("thinkingAdaptive", str);
	}
...
const THEME_KEY = Symbol.for("@porcupineai/coding-agent:theme");
