...
import { DEFAULT_SUBAGENT_MAX_STEPS, normalizeContextWindow } from "@porcupineai/agent-core";
...
export interface CompactionSettings {
	...
	/** Fixed reserve. Unset = 20% of model context window (80/20 rule). */
	reserveTokens?: number;
	/** Fixed keep-recent. Unset = 20% of model context window (clamped 8k–80k). */
...
export interface VoiceSettings {
	/** Push-to-talk: hold the space bar (when the editor is empty) to record. Default true. */
	pushToTalk?: boolean;
	/** Explicit microphone device index (macOS avfoundation). Unset = auto-select a real mic (skips BlackHole/virtual). */
	inputDevice?: number;
	/** Speech-to-text model id (Moonshine). "tiny" (fast) or "base" (accurate). Default "tiny". */
	sttModel?: string;
	/** Text-to-speech voice for Kokoro, e.g. "af_heart", "am_michael", "af_sky". Default "af_heart". */
	ttsVoice?: string;
	/** Speak the agent's response aloud after each turn. Default true. */
	autoSpeak?: boolean;
}
...
export class SettingsManager {
	...
	/** Explicit fixed reserve, or undefined to use the 80/20 context-window default. */
	getCompactionReserveTokens(): number | undefined {
		const value = this.settings.compaction?.reserveTokens;
		return typeof value === "number" && Number.isFinite(value) && value >= 0 ? value : undefined;
	}

	/** Explicit fixed keep-recent, or undefined to scale 20% of the model context window (clamped 8k–80k). */
	getCompactionKeepRecentTokens(): number | undefined {
		const value = this.settings.compaction?.keepRecentTokens;
		return typeof value === "number" && Number.isFinite(value) && value > 0 ? value : undefined;
	}
	...
	getVoiceSettings(): {
		...
		return {
			pushToTalk: this.settings.voice?.pushToTalk ?? true,
			sttModel: this.settings.voice?.sttModel ?? "tiny",
			ttsVoice: this.settings.voice?.ttsVoice ?? "af_heart",
			autoSpeak: this.settings.voice?.autoSpeak ?? true,
			inputDevice: this.settings.voice?.inputDevice,
		};
	}
	...
	getClearOnShrink(): boolean {
		...
		return getProductEnvironment("CLEAR_ON_SHRINK") === "1";
	}
	...
	getShowHardwareCursor(): boolean {
		return this.settings.showHardwareCursor ?? getProductEnvironment("HARDWARE_CURSOR") === "1";
	}
