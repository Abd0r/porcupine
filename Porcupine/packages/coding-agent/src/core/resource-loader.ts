...
function loadContextFileFromDir(dir: string): { path: string; content: string } | null {
	const candidates = ["AGENTS.md", "AGENTS.MD", "Agents.md", "CLAUDE.md", "CLAUDE.MD"];
...
/** First existing markdown candidate under dir (trusted project or global agent home). */
...
/** Always-on identity layers (appended; never replace the default tool prompt). */
const PERSONALITY_PROMPT_NAMES = [
	"PERSONALITY.md",
	"PROMPT.md",
	// legacy mixed-case fallbacks
	"Personality.md",
...
const APPEND_SYSTEM_PROMPT_NAMES = [
	"APPEND_SYSTEM.md",
	"APPEND_PROMPT.md",
	// legacy
	"Append_System.md",
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	async reload(options?: ResourceLoaderReloadOptions): Promise<void> {
		...
		// Built-in skills are the lowest-precedence product defaults: user/project
		// skills are loaded first by loadSkills(), so they retain collision priority.
		// Do not load them under --no-skills, which must disable all default discovery.
		const bundledSkillPaths = !this.noSkills && existsSync(getBundledSkillsDir()) ? [getBundledSkillsDir()] : [];
		const skillPaths = this.noSkills
			? this.mergePaths(cliEnabledSkills, this.additionalSkillPaths)
			: this.mergePaths(
					this.mergePaths([...cliEnabledSkills, ...enabledSkills], bundledSkillPaths),
		...
		if (!appendSources) {
			...
			// Personality / Prompt.md first (identity), then APPEND_SYSTEM.md
			discovered.push(...this.discoverPersonalityFiles());
			const discoveredAppendSystemPromptFile = this.discoverAppendSystemPromptFile();
			if (discoveredAppendSystemPromptFile) {
				discovered.push(discoveredAppendSystemPromptFile);
			}
			// De-dupe paths (Prompt.md might equal Personality in weird setups)
			const seen = new Set<string>();
			appendSources = discovered.filter((p) => {
	...
	private discoverSystemPromptFile(): string | undefined {
		if (this.settingsManager.isProjectTrusted()) {
			const projectPath = firstExistingFile(join(this.cwd, CONFIG_DIR_NAME), [...SYSTEM_PROMPT_NAMES]);
			if (projectPath) return projectPath;
		}

		return firstExistingFile(this.agentDir, [...SYSTEM_PROMPT_NAMES]);
	}

	private discoverAppendSystemPromptFile(): string | undefined {
		const names = [...APPEND_SYSTEM_PROMPT_NAMES];
		if (this.settingsManager.isProjectTrusted()) {
			const projectPath = firstExistingFile(join(this.cwd, CONFIG_DIR_NAME), [...names]);
			if (projectPath) return projectPath;
		}

		return firstExistingFile(this.agentDir, [...names]);
	}

	/** Always-on personality layer (does not replace the default tool prompt). */
