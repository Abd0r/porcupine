...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, undefined/false if porcupine-generated (backward compatible) */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, false if porcupine-generated */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.porcupine/agent/sessions/.
 */
...
async function buildSessionInfo(filePath: string): Promise<SessionInfo | null> {
	try {
		...
		for await (const line of rl) {
			...
			// Index compaction summaries into the search haystack so old content
			// remains findable after context compression. Without this, once the
			// conversation is compacted, keywords that only existed in the pruned
			// portion become invisible to session_search.
			if (entry.type === "compaction") {
				const summary = typeof entry.summary === "string" ? entry.summary : "";
				if (summary) {
					allMessages.push(summary);
				}
				const compactTime = new Date(entry.timestamp).getTime();
				if (!Number.isNaN(compactTime)) {
					lastActivityTime = Math.max(lastActivityTime ?? 0, compactTime);
				}
				continue;
			}
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// non-empty but did not parse as a porcupine session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid porcupine session: ${explicitPath}`);
				}
				this.newSession();
	...
	/**
	 * Write every in-memory entry to disk immediately — even when no assistant
	 * message exists yet (normal append defers the first flush until then).
	 * Used by `/refresh` so the session file is always reopenable.
	 * @returns Absolute session file path, or undefined if not a persisted session.
	 */
	forceFlushToDisk(): string | undefined {
		if (!this.persist) {
			return undefined;
		}
		if (!this.sessionFile) {
			// Allocate a path the same way first append would.
