...
import type { ImageContent, Model, Provider, ProviderHeaders } from "@porcupineai/ai";
...
export class ExtensionRunner {
	...
	async emitBeforeAgentStart(
		...
		if (messages.length > 0 || systemPromptModified || promptModified) {
			return {
				messages: messages.length > 0 ? messages : undefined,
				systemPrompt: systemPromptModified ? currentSystemPrompt : undefined,
				prompt: promptModified ? currentPrompt : undefined,
			};
		}
