...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@porcupineai/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@porcupineai/coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown Porcupine and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Porcupine loaded without re-discovering resources. */
...
export interface BeforeAgentStartEventResult {
	...
	systemPrompt?: string;
	/** Replace the user prompt text for this turn. Extensions use this to inject extracted/transcribed content before the main model sees the turn. */
	prompt?: string;
}
...
export interface ExtensionAPI {
	...
	/** Register a transformer for user and assistant Markdown before Porcupine renders it in the interactive transcript. */
...
export interface ProviderModelConfig {
	...
	reasoning: boolean;
	/** Maps thinking levels to provider/model-specific values; null marks a level unsupported. */
...
/**
 * Action implementations for api.* (ExtensionAPI) methods.
