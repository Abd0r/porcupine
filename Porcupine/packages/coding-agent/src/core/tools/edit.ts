...
export interface EditToolOptions {
	...
	/** Approval gate for a file mutation. Returning false blocks the edit. */
			...
			return withFileMutationQueue(absolutePath, async () => {
				if (
					...
					!(await confirmMutation(
						"File edit",
						`Apply ${edits.length} edit${edits.length === 1 ? "" : "s"} to ${path}?`,
					))
				) {
					throw new Error(`User denied edit to ${path}.`);
				}
