...
export interface WriteToolOptions {
	...
	/** Approval gate for a file mutation. Returning false blocks the write. */
	confirmMutation?: (title: string, message: string) => Promise<boolean>;
}
	...
	return {
		...
		async execute(
			...
			return withFileMutationQueue(absolutePath, async () => {
				if (
					confirmMutation &&
					!(await confirmMutation("File write", `Write ${content.length} bytes to ${path}?`))
				) {
					throw new Error(`User denied write to ${path}.`);
				}
