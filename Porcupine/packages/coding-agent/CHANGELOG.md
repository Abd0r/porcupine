...
- Added `/sandbox [on|off|status]`: one-command Gondolin micro-VM isolation — `on` copies the bundled Gondolin extension into the agent home, installs `@earendil-works/gondolin`, registers it in settings, and hot-reloads so built-in tools route into the VM; `status` checks Node/QEMU/VM state.
- Added remote bridges for Discord and iMessage alongside Telegram, sharing one contract (messages run on the shared session, responses return to the originating channel, provenance-matched forwarding, TUI↔channel racing for confirm/select/input with first-response-wins). Discord: `PORCUPINE_DISCORD_TOKEN` + `PORCUPINE_DISCORD_ALLOW`, zero-dependency gateway (built-in WebSocket, auto-resume, 429 backoff), ✅/❌ and numbered reactions. iMessage (macOS): `PORCUPINE_IMESSAGE_ALLOW`, AppleScript polling, text-based APPROVE/DENY + numbered replies.
- Added richer status-strip activity chips: `capability_search` infers `Searching for skills` / `Searching for tools` from the query, `web_search`/`web_extract` show 🌐 `Searching` / 📄 `Extracting`, unmapped tools show 🧰 `Using <tool>`, `subagent` shows 🤖 `Using Sub Agent`, and messaging shows 📨 `Sending message` then ✉️ `Sent message`. Chips now appear the moment the model streams a tool call (not after `tool_execution_start`).
- Added live sub-agent activity to the status strip: `🤖(activity)` chips in slot order (position 1 = first sub-agent), comma-joined for parallel workers (`🤖(📄 Extracting, 🌐 Searching)`), fully dynamic up to `subagent.maxConcurrent`. Sub-agent progress events now carry tool args and a thinking/tool phase (`SubagentRunInfo.lastToolArgs` + `phase`).
- Added a live sub-agent counter to the footer, left of the provider: `🧵 2/3 (opencode-go) deepseek-v4-flash • ⚡ Auto • high`, updating on every sub-agent event.
- Added `--headless` task mode: run a prompt to completion, print the final report, exit `0` on success / `1` on error or abort (CI-friendly; honors saved trust or `--approve`).
- Expanded the sub-agent tool stack to the whole stack minus agent-level tools (`subagent`, `ask_question`, `computer_use`, `tasks`, `projects`); sub-agents now get `capability_search`, `session_search`, and `mcp_resources`, so the full skill catalog is reachable. Raised the default step budget to `DEFAULT_SUBAGENT_MAX_STEPS = 120`.
- Added `stop_subagent`: the main agent can stop a running sub-agent directly (one by id, or all) — a stopped run reports `⏹ cancelled` instead of completing.
- Added the update system: startup update check (npm registry for the installed package, GitHub releases fallback, 24h cache) shows `🆕 vX.Y.Z available` beside the version in the header + a notification; `/update` forces a fresh check; `porcupine update [--yes]` checks and installs; `porcupine sync [--force]` refreshes the shipped agent-home prompt files without clobbering user edits (hash-tracked). New settings: `updateCheck` (default true) and `updateCheckIntervalHours` (default 24).
- Added `docs/stacks.md` (the capability tree: how every tool/skill is organized and discovered) and `docs/subagents.md` (parallel workers, WoT, budgets, activity chips), linked from the docs index.

- Added a `subagent` tool: the agent can spawn an isolated sub-agent (fresh context window 128K–256K, curated tool set, hard step/token budget) to offload self-contained work, receiving a structured report back. Sub-agents run on their own model (`subagent.model`, cheap/small recommended like `opencode-go/deepseek-v4-flash`), share the cwd + permission policy, cannot spawn sub-agents, and are capped at `subagent.maxConcurrent` (default 1).
- Added the sub-agent TUI panel: while a sub-agent runs, the screen splits 2/3 (main agent) – 1/3 (live sub-agent progress), auto-hiding when idle.
- Added `subagent` settings (`model`, `maxSteps` default 120, `contextWindow` default 256000 clamped 128K–256K, `maxConcurrent` default 3).
- Added the `runSubagent` runner in the agent package (`packages/agent/src/porcupine/subagent.ts`) with step/context budget enforcement via run abort.

- Added Auto Mode autonomy: enabling `/auto` injects a `<porcupine_auto_mode>` system-prompt directive and loads the `auto-mode` skill, instructing the agent to run safe steps, recover from ordinary failures, and verify results on its own instead of pausing for a human. Hardline destructive commands remain fail-closed.
- Added the `auto-mode` skill (`skills/meta/auto-mode/SKILL.md`) describing autonomous operating rules and hardline boundaries for Auto Mode.
- Added `/projects [query]`, a slash command that lists and searches canonical `Project/<name>` workspaces by README.md and STATUS.md.
...
- Added an agent-facing `tasks` tool (12 actions: `list`, `create`, `show`, `run`, `pause`, `resume`, `cancel`, `schedule_*`) so the model can manage durable local tasks and cron schedules; queued runs execute through the idle drain and record their real results in the append-only run history.
- Added a read-only `projects` tool that lists, searches, and views `Project/<name>/` workspaces (created with the `project-hygiene` skill) with path-traversal protection.
- Added `/learning history`, which renders the append-only learning event log (`~/.porcupine/agent/learning/events.jsonl`) newest-first instead of duplicating the graph view.
- Added a 3-minute dialog timeout to `ask_question`: an unanswered question returns a distinct `timedOut` result (never a cancel) so the agent may re-ask or continue working instead of stalling or looping.
- Added agent-home prompt templates (`PROMPT.md`, `AGENTS.md`, `PERSONALITY.md`, `SYSTEM.md`, `APPEND_SYSTEM.md`) to the published package and binary — the identity layers referenced by the docs now actually ship.

### Fixed

- Added **WoT (Web of Thoughts) live sub-agent messaging**: sub-agents sharing a `peerGroup` can message each other and the main agent **instantly** (injected into the recipient's live context — steered into a running loop or a fresh turn when idle, never gated on reports). Main→Sub via a new `send_to_subagent` tool; sub→main folds into context immediately with dedupe against the report fold-in; every routed message is audited on the bus (`session.subagentMessageBus`). Main-agent-gated: no `peerGroup` = fully isolated.
- Added the **multi-slot sub-agent panel**: `🧵 Sub-agents 2/3` with one line per running sub-agent (task label, steps, last tool) and dim idle slots — replaces the single-latest panel.
- Changed sub-agent report delivery: the report is now injected into the session **instantly** when the sub-agent finishes — steered into the running turn if the parent is mid-task, or a fresh turn is started if the session is idle — instead of waiting for the next user turn (`_pendingNextTurnMessages`). The main agent can fold the result into its work without a user prompt.
- Fixed `subagent.maxConcurrent` being ignored: the concurrency guard was a hardcoded boolean (max 1) so raising the setting had no effect. The tool now reads the live run count and enforces the configured capacity (`getActiveSubagentRuns`).
- Changed `subagent.maxConcurrent` default from 1 to **3** (user-configurable via `~/.porcupine/agent/settings.json` or by asking the agent).
- Added sub-agent cancellation: the `subagent` tool now wires an `AbortSignal` into each background run. The session exposes `cancelSubagent(id)` / `cancelAllSubagents()`; user Escape (with an idle editor) stops running sub-agents, and session abort/teardown cancels them too. Cancelled runs report `⏹ cancelled` instead of silently completing.
- Added a **dynamic task graph for model-led turns**: previously the footer task tracker and chat `TaskGraphComponent` only appeared for explicit plan requests (`/plan`, "make a plan", …). Now ordinary multi-step turns build a live graph from actual tool calls — consecutive same-tool calls collapse into one step (a phase), steps are capped at 12 for a compact view, and the tracker/graph animate as tools execute (`ensureDynamicStep` / `resetDynamicGraph` in `session-orchestrator.ts`). Explicit plan graphs remain authoritative when present.
...
- Fixed the interactive-mode bash gate and file-mutation confirmations capturing the interaction mode at session build: `/modes` and `/auto` switches now reach the guard live, without a session rebuild.
- Fixed the interactive TUI never wiring the session confirm callback — Ask mode blocked every bash command with an error instead of prompting, Normal-mode flagged commands were blocked instead of asking, and Ask-mode file edits never confirmed. The TUI now prompts through its selector and headless Ask fails closed.
- Fixed `/auto` driving a separate internal flag while the prompt, bash gate, and footer keyed off the interaction mode; both now share one canonical state.
- Fixed `capability_search` hiding bundled product skills (only user/project skills were visible).
- Fixed `ask_question` accepting unbounded questions/answers and rendering raw newlines in dialogs; questions, labels, and answers are now size-capped and sanitized, options map back to their clean labels, and cancels tell the agent not to re-ask.
- Fixed the autonomous capability-learning loop corrupting patched skills (old frontmatter nested under the new document's Procedure) and silently overwriting existing skills on create; a throwing learner now records a rejected result instead of crashing the turn.
- Fixed `porcupine-learning` skill stubs and `task-scheduler` orphan claims: a terminated process marks abandoned runs `unknown` on the next start and clears cron claims.
- Replaced Hermes tool names (`read_file`, `search_files`, `patch`, `write_file`, `terminal`) with the real Porcupine tools (`read`, `grep`, `find`, `edit`, `write`, `bash`) across seven skills.
- Fixed the Normal-mode footer badge alignment: the shield renders two characters wide, so `🛡️  Normal` keeps its double space.
