...
- Added `defaultInteractionMode` setting (`"ask" | "normal" | "auto"`, default `"normal"`): the interaction mode new sessions start in — e.g. start every session in Auto mode without typing `/auto`.
