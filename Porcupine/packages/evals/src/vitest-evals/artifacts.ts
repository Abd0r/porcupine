...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@porcupineai/evals:session" && artifact.type !== "@porcupineai/evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@porcupineai/evals:session" ? "sessions" : "sources";
